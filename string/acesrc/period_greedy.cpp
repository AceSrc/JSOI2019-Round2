...
int main() {
  ...
  int t = 0;
  static int idx[maxn], Stack[maxn];
  static vector<int> Discard[maxn];
  static bool die[maxn];

  int K = 100000000 / n;
  for (int i = 0; i < n; i++) {
      int last = n;
      for (int j = i - 1; j >= 0 &&  i - j <= K; j--) {
          if (sa.LCP(sa.rk[i], sa.rk[j]) >= i - j) {
            last = i + i - j - 1;
            break;
          }
      } 
      if (last != n) Discard[last].push_back(i);
      for (auto &v : Discard[i])
          die[v] = true;
