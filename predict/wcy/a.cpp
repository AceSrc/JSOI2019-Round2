...
void run()
{
    ...
    for(int i=sz-1;i>=0;i--)
    {
        ...
        for(int j=0;j<(int)changed.size();j++) tmp[changed[j]]|=tmp2[changed[j]],tmp2[changed[j]].reset();
...
int main()
{
    ...
    run();
    for(int i=0;i<n;i++) if(ans[i].test(i)) dead.set(i);
    for(int i=0;i<n;i++) printf("%d%c",dead.test(i)?0:n-1-(int)(ans[i]|dead).count(),i==n-1?'\n':' ');
    return 0;
}
